...
object Http4sTutorial extends IOApp {
  ...
  // Below QueryParamDecoderMatcher will extract a query param name director of type
  // String from URL
  ...
  def movieRoutes[F[_] : Monad]: HttpRoutes[F] = {
    ...
    HttpRoutes.of[F] {
          // +& will ask to look for another query param matcher after & in url
      ...
      case GET -> Root / "movies" / UUIDVar(movieId) / "actors" =>
        findMovieById(movieId).map(_.actors) match {
          ...
          case _ => NotFound(s"No movie with id $movieId")
        }
  ...
  val directorDetailsDB: mutable.Map[Director, DirectorDetails] =
    mutable.Map(Director("Zack", "Snyder") -> DirectorDetails("Zack", "Snyder","Zombie"))
  ...
  def allRoutes[F[_]: Monad]: HttpRoutes[F] =
    movieRoutes[F] <+> directorRoutes[F] // <+> is from cats.syntax.semigroupk._
    // semigroupk is a higher kinded class for combine

  // returns 404 if no routes are found
